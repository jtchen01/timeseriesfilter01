...
knitr::opts_chunk$set(warnings=FALSE, error = FALSE, message = FALSE)
...
If we assume that the signal $y_t$ is observed at regular dates, we obtain:
...
We assume the price follows a lognormal distribution with stochastic movement.
...
In this manner, we can easily flip between the trend of log-price and the trend of price returns, since the relationship between $\mu_t$ and $R_t$ is equal to that between $\hat{x}_t $.


## Moving Average Filter

The simplist filter will be moving average, under the idea that through aggregation random ocillation is canceled out and only trend is left.
...
A simple example of moving average filter is shown as followed.
```{r}
...
## L2 filter
Generally we want our trend to follow the same movement of our time series data.

This is expressed in the function
$$\left\{\hat{x}_1,...,\hat{x}_n\right\}=\arg\min\frac{1}{2}\sum_{t=1}^{n}\left(y_t-\hat{x}_t\right)^2$$

However, we would like to impose some restrictions on our trend function so that it does not overfit to short-term, non-trend variations.

In L2 filter, we are trading-off between deviations from trend and smoothness of trend. An desirable trend attribute is that it moves along with the "big picture" of time series while less respondent to any short-term volatility.
...
If we consider the $\mu$ is not constant, we can define the following objective function, as a tradeoff between errors and smoothness.
...
Solving this problem and we can have a restricted fitted line that represents trend of the L2 filter.
...
The dynamic of the Kalman filter system can be expressed as
$$\begin{cases}R_t=\mu_t+\sigma_{\zeta}\zeta_t\\\mu_t=\mu_{t-1}+\sigma_{\eta}\eta_t\end{cases}$$

The trend of return itself is stochastic and autocorrelated, while the return adds another error term.

The estimation of $\mu_t$ can be defined as
$$\hat{\mu}_{t|t-1}=\mathbb{E}_{t-1}\left[\mu_t\right]$$

The variance of $\mu_t$, called $P_t$ is defined as
$$P_{t|t-1}=\mathbb{E}_{t-1}\left[\left(\hat{\mu}_{t|t-1}-\mu_t\right)^2\right]$$

Therefore we can define an update process that incorporates the latest information that
$$\hat{\mu}_{t+1|t}=(1-K_t)\hat{\mu}_{t|t-1}+K_tR_t$$
in which
$$K_t=\frac{P_{t|t-1}}{P_{t|t-1}+\sigma_{\zeta}^2}$$

In practice, we can use this method of solving the equation in this single-variate situation:
...
If we wrote the formula as this, we can set $\lambda =-\ln(1-\kappa)$

Then the prediction of this single time series can be rewritten as
$$\hat{\mu}_t =\left(1-e^{-\lambda}\right)\sum_{i=0}^{\infty} e^{-\lambda i} R_{t-i}$$
which is a exponential-weighted average.

And the trend of log-price can be written as
$$\hat{x}_t = \left(1-e^{-\lambda}\right)\sum_{i=0}^{\infty} e^{-\lambda i}y_{t-i}$$

Deriving first derivatives, we have
...
## Loess regression
For the local polynomial regressions like
$$\begin{align}y_t & \,=\, f(t) \,+\, \epsilon_t \\& \,=\, \beta_0(\tau) \,+\, \sum_{j=1}^{p}\beta_j(\tau)(\tau \, - t)^j \,+\,\epsilon_t\ \\\end{align}$$
they try to use basis functions with order more than 1 to fit some datasets that have no apparent relationship.

To avoid overfitting, loess regression first try to get the residuals $\hat{\epsilon_t}$, then  compute $\delta_t \,=\, (1 - \mu_t^2) \cdot 1\{|\mu_t|\leq 1\}$ with $\mu_t = \hat{\epsilon}_t / (6\, \text{median}|\hat{\epsilon}|)$ 
and run a second kernel regression with weightings $\delta_t w_t$.

In this way it effectively underweights points that are far away from the mainstream "trend".

An example shows further detail about it.
```{r results="hide", warning=FALSE, error=FALSE, message=FALSE}
...
The inclusion of L1 term results in the fitted curve to be locally linear, resulting in kinks among differing local trends.
```{r results="hide", warning=FALSE, error=FALSE, message=FALSE}
...
## Wavelet filter
Wavelet filter originates from Fourier transformation.
...
It decomposes time serieses into some cyclical functions. 

However they face difficulties when trends reverse from one specific period to another.
...
The process of Wavelet function generally follows:
...
An example of the Wavelet method is:
```{r results="hide", warning=FALSE, error=FALSE, message=FALSE}
# knitr::opts_chunk$set(echo = FALSE, warnings=FALSE, error = FALSE, message = FALSE)
...
We want to determine the best moving average window to predict future market prices.
...
With Kalman filter we would like to get the best lambda through time series cross validation as well.

In this case the optimal lambda value is 0.12.
...
For the L1 filter, it we set the error term to minimum, clearly lambda term should be minimum.

In the following example, we choose 0.001 as the optimal lambda.
...
We implement filters with optimal parameters that we've discussed so far and examine the result.

Five filters are used to extract trend from SP500 data from 2000 to 2010 and then to predict the next 5 year daily price. Once the filter responded a positive trend we will long the index while a negative trend will leads to shorting the index.

A result of five filter strategy is listed.
...
[3] Wavelet Methods in Statistics with R, 
